import 'dart:ui';

class Colours {
  const Colours._();

  // #EDF8FF #FDC1E8 #FDFAE1 #FFFFFF
  static const gradient = [
    Color(0xFFEDF8FF),
    Color(0xFFFDC1E8),
    Color(0xFFFDFAE1),
    Color(0xFFFFFFFF),
  ];

  // #458CFF
  static const primaryColour = Color(0xFF458CFF);

  // #757C8E
  static const neutralTextColour = Color(0xFF757C8E);

  // #D3D5FE
  static const physicsTitleColour = Color(0xFFD3D5FE);

  // #FFEFDA
  static const scienceTitleColour = Color(0xFFFFEFDA);

  // #FFE4F1
  static const chemistryTitleColour = Color(0xFFFFE4F1);

  // #CFC5FC
  static const mathTitleColour = Color(0xFFCFC5FC);

  // #DDAFD6
  static const languageTitleColour = Color(0xFFDDAFD6);

  // #D5BEFB
  static const literatureTitleColour = Color(0xFFD5BEFB);

  // #FF5C5C
  static const redColour = Color(0xFFFF5C5C);

  // #2BCA6C
  static const greenColour = Color(0xFF2BCA6C);

  // #F4F5F6
  static const chatFieldColour = Color(0xFFF4F5F6);

  // #E8E9EA
  static const chatFieldColourDarker = Color(0xFFE8E9EA);

  static const currentUserChatBubbleColour = Color(0xFF2196F3);

  static const otherUserChatBubbleColour = Color(0xFFEEEEEE);

  static const currentUserChatBubbleColourDarker = Color(0xFF1976D2);

  static const otherUserChatBubbleColourDarker = Color(0xFFE0E0E0);
}
